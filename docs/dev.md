...
2. Run `npm install`.
3. Copy `.env.example` to `.env` and set `BALLDONTLIE_API_KEY` for scripts and GitHub Actions.
4. Backfill once for a season (creates `data/games-<season>.json`):
   - `npm run backfill:games -- 2025`

## Environment variables

- `BALLDONTLIE_API_KEY`: required for scripts and GitHub Actions that pull from BALLDONTLIE. The production runtime does not use this env var.
